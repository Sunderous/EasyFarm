    ...
    sub_commands = {
        ...
        domain = function(current_activity, zone, p, settings)
            ...
            if #args == 0 or args[1] == 'enter' then
                ...
                return actions
            elseif args[1] == "mollifier" then
                log('Check Mollifier')

                local key_items= windower.ffxi.get_key_items()

                local has_mollifier = false;
                ...
                if has_mollifier then
                    ...
                    packet["Zone"] = zone
                    packet["Menu ID"] = menu
                    actions:append(T{packet=packet, description='cancel menu', message='Already has mollifier'})        
                    return actions        
                end

                local silt_stock = p["Menu Parameters"]:unpack('i', 5)

                if silt_stock < 500 then
                    packet = packets.new('outgoing', 0x05B)
                    packet["Target"] = npc.id
                    packet["Option Index"] = 0
                    packet["_unknown1"] = 16384
                    ...
                    packet["Zone"] = zone
                    packet["Menu ID"] = menu
                    actions:append(T{packet=packet, description='cancel menu', message='Not enough silt for mollifier'})        
                    return actions        
                end
                ...
                packet["Zone"] = zone
                packet["Menu ID"] = menu
                actions:append(T{packet=packet, wait_packet=0x05C, description='key items'})

                packet = packets.new('outgoing', 0x05B)
                packet["Target"] = npc.id
                packet["Option Index"] = 2564
                packet["_unknown1"] = 0
                packet["Target Index"] = npc.index
                packet["Automated Message"] = true
                packet["_unknown2"] = 0
                packet["Zone"] = zone
                packet["Menu ID"] = menu
                actions:append(T{packet=packet, wait_packet=0x05C, description='get mollifier'})
                ...
                packet["Zone"] = zone
                packet["Menu ID"] = menu
                actions:append(T{packet=packet, wait_packet=0x05C, description='return to key items'})
                ...
                packet["Zone"] = zone
                packet["Menu ID"] = menu
                actions:append(T{packet=packet, wait_packet=0x05C, expecting_zone=false, delay = 1, description='exit menu'})    
